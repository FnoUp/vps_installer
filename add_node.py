...
if not location:
    error("Локация не может быть пустой")

net_dev = ask("Сетевой интерфейс на ноде", "ens3")

host_uuid = ask("UUID хоста в Remnawave (из панели → Hosts)")
if not host_uuid:
    error("UUID не может быть пустым")

emoji_name = ask("Название для Telegram алертов (например: 🇫🇷 France)", f"{location.title()}")
...
info(f"TG имя:     {emoji_name}")
print("─" * 50)
print()

confirm = input("Всё верно? Добавляем? (y/n): ").strip()
if confirm.lower() != "y":
    print("Отмена.")
    sys.exit(0)
